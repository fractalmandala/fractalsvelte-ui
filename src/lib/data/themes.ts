...
import { isPublicTokenName, tokenDefaults } from '../styles/tokens.schema.js';
...
const coreTokenDefinitions: Array<[string, string, TokenMeta['category'], string]> = [
	['bg', 'App Canvas Base', 'surface', 'Deepest backdrop surface of the application'],
	['bg-surface', 'Card Surface', 'surface', 'Primary card and content container background'],
	['bg-raised', 'Raised Elements', 'surface', 'Elevated popovers, floating headers, and tooltips'],
	['bg-panel', 'Sidebars & Panels', 'surface', 'Navbars, lateral sidebars, and panels'],
	['bg-footer', 'Footer Surface', 'surface', 'Bottom footer area'],
	['bg-popover', 'Popover & Modals', 'surface', 'Dropdowns, popovers, and popup menus'],
	['bg-dialog', 'Dialog Surface', 'surface', 'Dialog card background'],
	['bg-terminal', 'Code & Terminal', 'surface', 'Terminal blocks and code views'],
	['bg-input', 'Form Inputs', 'surface', 'Input fields, textareas, and select boxes'],
	['bg-canvas', 'Canvas / Viewport', 'surface', 'Outer workspace canvas background'],
	['border', 'Standard Border', 'border', 'Primary borders, card outlines, and separators'],
	['border-subtle', 'Subtle Border', 'border', 'Faint dividers and inner item borders'],
	['text-primary', 'Primary Text', 'typography', 'Headings, titles, and high-contrast text'],
	['text-secondary', 'Secondary Text', 'typography', 'Body paragraphs, descriptions, and labels'],
	['text-muted', 'Muted Text', 'typography', 'Captions, timestamps, and metadata'],
	['text-inverse', 'Inverse Text', 'typography', 'Text on accent and dark surfaces'],
	['state-hover', 'Hover State', 'state', 'Background tint for interactive hover states'],
	['state-hover-subtle', 'Subtle Hover', 'state', 'Gentle hover state on list items'],
	['state-selected', 'Selected State', 'state', 'Active tab and selected-item highlight'],
	['theme-color', 'Brand Accent', 'accent', 'Primary brand action colour and highlights'],
	['theme-color-alt', 'Accent Alternate', 'accent', 'Hover and alternate brand accent colour']
];

export const CORE_TOKENS: TokenMeta[] = coreTokenDefinitions.map(([key, label, category, description]) => ({
	key,
	label,
	category,
	description,
	defaultVal: tokenDefaults[`--${key}` as keyof typeof tokenDefaults] ?? ''
}));

const RAW_THEMES: ThemeInfo[] = [
	{
...
const publicThemeTokenNames = new Set(Object.keys(tokenDefaults).map((name) => name.slice(2)));

export const THEMES: ThemeInfo[] = RAW_THEMES.map((theme) => ({
	...theme,
	tokens: Object.fromEntries(Object.entries(theme.tokens).filter(([name]) => publicThemeTokenNames.has(name) && isPublicTokenName(`--${name}`)))
}));
