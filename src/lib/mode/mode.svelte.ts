import { transition, type TransitionSpec } from './transitions.js';
...
class ModeStore {
	 ...
	 */
	async setLight(spec?: TransitionSpec): Promise<void> {
		const swipe: TransitionSpec = {
			kind: 'wipe',
			direction: 'up',
			duration: 520,
		...
		await this.set('light', swipe);
	}

	/**
	 * Switch to dark mode.
	 */
	async setDark(spec?: TransitionSpec): Promise<void> {
		const swipe: TransitionSpec = {
			kind: 'wipe',
			direction: 'down',
			duration: 520,
